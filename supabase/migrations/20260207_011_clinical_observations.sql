...
CREATE INDEX IF NOT EXISTS idx_clinical_obs_patient ON clinical_observations(patient_id);
CREATE INDEX IF NOT EXISTS idx_clinical_obs_practice ON clinical_observations(practice_id);
CREATE INDEX IF NOT EXISTS idx_clinical_obs_type ON clinical_observations(observation_type);
CREATE INDEX IF NOT EXISTS idx_clinical_obs_effective_date ON clinical_observations(effective_date DESC);
CREATE INDEX IF NOT EXISTS idx_clinical_obs_code ON clinical_observations(code);

-- Composite indexes for common queries
CREATE INDEX IF NOT EXISTS idx_clinical_obs_patient_type ON clinical_observations(patient_id, observation_type);
CREATE INDEX IF NOT EXISTS idx_clinical_obs_patient_date ON clinical_observations(patient_id, effective_date DESC);
CREATE INDEX IF NOT EXISTS idx_clinical_obs_patient_code ON clinical_observations(patient_id, code);
CREATE INDEX IF NOT EXISTS idx_clinical_obs_practice_type_date ON clinical_observations(practice_id, observation_type, effective_date DESC);

-- Partial index for non-deleted records
CREATE INDEX IF NOT EXISTS idx_clinical_obs_not_deleted ON clinical_observations(id) WHERE deleted_at IS NULL;

-- JSONB index for value searches
CREATE INDEX IF NOT EXISTS idx_clinical_obs_value ON clinical_observations USING GIN (value);
...
DROP POLICY IF EXISTS "demo_clinical_obs_all" ON clinical_observations;
...
DROP POLICY IF EXISTS "admin_clinical_obs_include_deleted" ON clinical_observations;
...
DROP TRIGGER IF EXISTS update_clinical_observations_updated_at ON clinical_observations;
