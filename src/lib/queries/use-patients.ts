...
async function createPatientApi(data: CreatePatientData): Promise<{ patient?: Patient; duplicate?: DuplicatePatientResponse }> {
...
async function createPatientForcedApi(data: CreatePatientData): Promise<Patient> {
  const res = await fetch("/api/patients?force=true", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...data, force: true }) });
  const result = await res.json();
  if (!res.ok) throw new Error(result.error || "Failed to create patient");
  return result.patient;
}

async function updatePatientApi(patientId: string, practiceId: string, data: UpdatePatientData): Promise<Patient> {
  const res = await fetch(`/api/patients/${patientId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...data, practiceId }) });
  const result = await res.json();
  if (!res.ok) throw new Error(result.error || "Failed to update patient");
  return result.patient;
}

async function updatePatientStatusApi(patientId: string, practiceId: string, data: UpdatePatientStatusData): Promise<Patient> {
  const res = await fetch(`/api/patients/${patientId}/status`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...data, practiceId }) });
  const result = await res.json();
...
async function archivePatientApi(patientId: string, practiceId: string): Promise<void> {
  const res = await fetch(`/api/patients/${patientId}`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ practiceId }) });
  if (!res.ok) { const result = await res.json(); throw new Error(result.error || "Failed to archive"); }
}
...
export function useCreatePatient() {
  ...
  return useMutation({
    ...
    onSuccess: (result, variables) => { if (result.patient) queryClient.invalidateQueries({ queryKey: patientKeys.list(variables.practiceId) }); },
  });
}
...
export function useCreatePatientForced() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => { queryClient.invalidateQueries({ queryKey: patientKeys.list(variables.practiceId) }); },
  });
}
...
export function useUpdatePatient() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ patientId, practiceId = DEMO_PRACTICE_ID, data }: { patientId: string; practiceId?: string; data: UpdatePatientData }) => updatePatientApi(patientId, practiceId, data),
    onMutate: async ({ patientId, data }) => {
      ...
      if (prev) queryClient.setQueryData<Patient>(patientKeys.detail(patientId), { ...prev, first_name: data.firstName ?? prev.first_name, last_name: data.lastName ?? prev.last_name, updated_at: new Date().toISOString() });
      return { prev };
    },
    onError: (_, { patientId }, ctx) => { if (ctx?.prev) queryClient.setQueryData(patientKeys.detail(patientId), ctx.prev); },
    onSettled: (_, __, { patientId, practiceId }) => { queryClient.invalidateQueries({ queryKey: patientKeys.detail(patientId) }); queryClient.invalidateQueries({ queryKey: patientKeys.list(practiceId || DEMO_PRACTICE_ID) }); },
  });
}
...
export function useUpdatePatientStatus() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ patientId, practiceId = DEMO_PRACTICE_ID, data }: { patientId: string; practiceId?: string; data: UpdatePatientStatusData }) => updatePatientStatusApi(patientId, practiceId, data),
    onSuccess: (_, { patientId, practiceId }) => { queryClient.invalidateQueries({ queryKey: patientKeys.detail(patientId) }); queryClient.invalidateQueries({ queryKey: patientKeys.list(practiceId || DEMO_PRACTICE_ID) }); },
  });
}
...
export function useArchivePatient() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ patientId, practiceId = DEMO_PRACTICE_ID }: { patientId: string; practiceId?: string }) => archivePatientApi(patientId, practiceId),
    onSuccess: (_, { patientId, practiceId }) => { queryClient.removeQueries({ queryKey: patientKeys.detail(patientId) }); queryClient.invalidateQueries({ queryKey: patientKeys.list(practiceId || DEMO_PRACTICE_ID) }); },
  });
}
