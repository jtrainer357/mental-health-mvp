...
export async function logAudit(entry: AuditLogEntry): Promise<void> {
  try {
    ...
    // Try to call the RPC function if available, otherwise fall back to console logging
    // The audit_logs table and log_audit_event function are created by migration
    // but the Supabase types haven't been regenerated yet
    try {
      // Use type assertion to call the RPC function
      ...
      // If RPC fails (e.g., function doesn't exist yet), fall back to console logging
      console.log("[Audit]", JSON.stringify(auditRecord));
    }
  } catch (error) {
