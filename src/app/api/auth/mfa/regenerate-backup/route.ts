  ...
  try {
    ...
    if (!isValid) {
      await supabase.from("mfa_audit_log").insert({
        user_id: session.user.id, action: "backup_regenerated", ip_address: ip, user_agent: ua,
    ...
    await supabase.from("user_mfa").update({ backup_codes: hashedBackupCodes }).eq("user_id", session.user.id);
    await supabase.from("mfa_audit_log").insert({ user_id: session.user.id, action: "backup_regenerated", ip_address: ip, user_agent: ua, success: true });

    return NextResponse.json({ success: true, backupCodes: formatBackupCodes(backupCodes), message: "New backup codes have been generated." });
  } catch (error) {
