...
import type { AppointmentInsert, AppointmentReminderInsert } from "@/src/lib/supabase/types";
...
async function checkConflicts(
  ...
  let query = supabase
    .from("appointments")
    .select(
      `id, date, start_time, end_time, patient:patients(first_name, last_name)`
    )
    ...
    if (recurring) {
      ...
      for (const futureDate of futureDates) {
        ...
        if (futureConflicts.length > 0) {
          log.warn("Skipping conflicting recurring date", { date: futureDate, conflicts: futureConflicts });
          continue;
        }

        const recurringAppt: AppointmentInsert = { ...appointment, date: futureDate };
