...
export interface SignAndLockDialogProps {
  open: boolean;
...
/**
 * Sign and Lock Confirmation Dialog
 * Confirms provider intent to lock the note with e-signature
 */
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-center">
            This action cannot be undone. Please review before signing.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          {willBeLateEntry && (
            <div className="flex items-start gap-3 rounded-lg border border-warning/50 bg-warning/10 p-3">
              ...
              <div>
                <Text weight="medium" className="text-warning">
                  Late Entry Notice
                </Text>
                <Text size="sm" muted>
                  This note is being signed more than 24 hours after the session
                  date. It will be automatically marked as a late entry.
                </Text>
              </div>
            </div>
          )}
          ...
          <div className="rounded-lg bg-surface-secondary p-4">
            ...
            <ul className="mt-2 space-y-1.5 text-sm text-foreground-muted">
              <li className="flex items-start gap-2">
                <SecurityCheckIcon size={16} className="mt-0.5 shrink-0" />
                This note is accurate and complete to the best of your knowledge
              </li>
              <li className="flex items-start gap-2">
                <LockIcon size={16} className="mt-0.5 shrink-0" />
                The note will be locked and cannot be edited
              </li>
              <li className="flex items-start gap-2">
                <SecurityCheckIcon size={16} className="mt-0.5 shrink-0" />
                Only addendums can be added after signing
              </li>
            </ul>
          </div>
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          <Button
            onClick={handleSign}
            disabled={isLoading || isSubmitting}
            className="min-h-[44px] gap-2 bg-accent hover:bg-accent/90"
          >
            <LockIcon size={16} />
            {isSubmitting ? 'Signing...' : 'Sign & Lock'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
